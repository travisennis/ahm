...
| **Repository-local lock** | A filesystem lock under `.agents/.lock/` (or `.ahm/.lock/` in ref-backed repositories) used to serialize `ahm task create` ID allocation across concurrent invocations. Narrower than broad advisory locking; adopted in ADR 010. | `internal/ahm/lock.go` | [ADR 010](../adr/010-task-create-id-allocation-lock.md), [spec §Workflow State](workflow-spec.md) |
