...
| **Workflow** | The set of directories, templates, source records, and generated indexes under `.agents/` that `ahm` manages. Includes task queues, research notes, ExecPlans, and agent skills. | `.agents/` (on disk) | [spec §Workflow State](workflow-spec.md), [ARCHITECTURE.md](../../ARCHITECTURE.md) |
...
| Term | Definition | Implements | See Also |
| ---- | ---------- | ---------- | -------- |
| **Managed template file** | A file owned by `ahm` — installed by `ahm init` and updated by `ahm upgrade` when the embedded template changes. Locally modified managed files are preserved and reported as conflicts (unless `--force`). Examples: `.agents/TASKS.md`, `docs/adr/README.md`, `.agents/skills/*/SKILL.md`. | `internal/templates/templates.go` (registry), `internal/ahm/install.go` (install/upgrade) | [spec §File Ownership Boundary](workflow-spec.md), [upgrade guide](../guides/workflow-upgrades.md) |
...
| **Create-only file** | A special ownership category: `ahm init` creates the file when it is missing, but `ahm` never overwrites an existing copy, even with `--force`. The only create-only file is `AGENTS.md`. | `internal/ahm/install.go` | [spec §File Ownership Boundary](workflow-spec.md), [ADR 002](../adr/002-advisory-agents-suggestions.md) |
...
| **Smoke test** | A live end-to-end test (`just smoke-agents`) that drives each installed agent through `ahm task work --complete` in a throwaway repo. Required after changes to agent arg builders, parsers, or orchestration. | `internal/ahm/task_work_smoke_test.go` | [testing guide](../guides/testing.md) |
