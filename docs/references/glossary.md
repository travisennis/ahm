...
| **Project-owned AGENTS.md** | A repository's own agent instruction file. `ahm init`, `ahm upgrade`, and `--force` never create, overwrite, or remove it. `ahm agents suggestions` can print advisory snippets for maintainers to adapt manually. | `internal/ahm/agents.go` | [spec §File Ownership Boundary](workflow-spec.md), [ADR 011](../adr/011-expose-agent-instructions-through-context-command.md) |
