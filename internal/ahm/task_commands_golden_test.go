...
func readAgentGolden(t *testing.T, name string) []byte {
	...
	if err != nil {
		t.Errorf("reading golden transcript (refresh with `just capture-agent-fixtures`): %v", err)
	}
	return data
}
...
func TestGoldenCakeWorkSessionID(t *testing.T) {
	...
	if err != nil {
		t.Errorf("parseCakeSessionID: %v", err)
	}
	if id == "" {
		t.Error("no session ID parsed from cake work golden")
	}
}
...
func TestGoldenCakeReviewFeedback(t *testing.T) {
	...
	if err != nil {
		t.Errorf("parseCakeReviewFeedback: %v", err)
	}
	if strings.TrimSpace(feedback) == "" {
		t.Error("no review feedback parsed from cake review golden")
	}
}
...
func TestGoldenCodexReviewFeedback(t *testing.T) {
	...
	if err != nil {
		t.Errorf("parseCodexReviewFeedback: %v", err)
	}
	if strings.TrimSpace(feedback) == "" {
		t.Error("no review feedback parsed from codex review golden")
	}
}
...
func TestGoldenCodexExecSessionID(t *testing.T) {
	...
	if err != nil {
		t.Errorf("parseCodexSessionID: %v", err)
	}
	if id == "" {
		t.Error("no session ID parsed from codex exec golden")
	}
}
...
func TestGoldenCodexResumeRoundTrip(t *testing.T) {
	...
	if err != nil {
		t.Errorf("parseCodexSessionID(exec): %v", err)
	}
	resumeID, err := parseCodexSessionID(readAgentGolden(t, "codex-resume.jsonl"))
	if err != nil {
		t.Errorf("parseCodexSessionID(resume): %v", err)
	}
	if resumeID == "" {
		t.Error("no session ID parsed from codex resume golden")
	}
	if resumeID != execID {
		t.Errorf("resume thread ID = %q, want exec thread ID %q", resumeID, execID)
	}
}
...
func TestGoldenProvenanceSidecars(t *testing.T) {
	...
	if err != nil {
		t.Errorf("reading golden directory: %v", err)
	}
	var goldens int
	...
	if goldens == 0 {
		t.Error("no golden transcripts found in testdata/agents")
	}
}
