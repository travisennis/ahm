...
func (m *metadata) UnmarshalJSON(data []byte) error {
	...
	for _, key := range []string{
		"version",
		"strict_acceptance",
		// Consume obsolete agent-selection and research keys without preserving
		// them in Extra so the next metadata write removes them.
		"default_work_agent",
		"taskWork",
		"projectDocs",
		"research",
...
// retiredGeneratedIndexes were regenerated by ahm while research notes and
// ExecPlans were record families. ahm no longer writes them, so any stale
// metadata ownership is relinquished while the files stay on disk as ordinary
// project files.
var retiredGeneratedIndexes = []string{
...
func relinquishRetiredGeneratedIndexes(meta *metadata) bool {
	return relinquishMetadataOwnership(meta, retiredGeneratedIndexes)
}
...
func (a *app) install(upgrade bool) error {
	...
	relinquishRetiredGeneratedIndexes(&meta)
	if !a.opts.dryRun {
