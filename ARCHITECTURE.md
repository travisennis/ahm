...
manages task and ADR records, reports live repository state, validates workflow
health, and regenerates deterministic indexes.

## System Boundaries

- `ahm` owns workflow installation, upgrades, validation, task/ADR lifecycle
  commands, and generated indexes.
...
| Install & upgrade | `internal/ahm/install.go` | `init`, `upgrade`, metadata, legacy instruction removal, generated index writes. |
...
| Tasks | `internal/ahm/tasks.go`, `task_commands.go`, `task_create.go`, `task_list.go`, `task_status.go`, `task_find.go`, `task_enum.go`, `task_comment.go`, `task_deps.go`, `task_migrate.go`, `task_acceptance.go` | Task model, parsing, rendering, all lifecycle commands, dependency management, acceptance checking. |
